...
def _upload(
    ...
    with requests.Session() as r:
        ...
        if "path" in result:
            ...
            result["url"] = f"{base_url}/" + result["path"]
            return result
        else:
...
def _upload_via_api(
    ...
    method = "/editPage" if path else "/createPage"
    ...
    if resp["ok"] is True:
        return {
            ...
            "url": f"{base_url}/" + resp["result"]["path"],
        }
...
class TelegraphPoster(object):
    ...
    def _api_request(self, method, params=None):
        ...
        resp = requests.get(
            f"{api_url}/{method}", params, headers={"User-Agent": self.user_agent}
        )
