# coding=utf8
import unittest
from html_telegraph_poster.upload_images import (
    upload_image,
    _get_mimetype_from_response_headers,
)
...
class UploadImageTest(unittest.TestCase):
    def test_upload(self):
        ...
        self.assertIn("http://graph.org/file/", telegraph_url)

    def test_upload_return_json(self):
        telegraph_response = upload_image(
            "http://httpbin.org/image/jpeg", return_json=True
        )
    ...
    def test_upload_from_file_object(self):
        ...
        with BytesIO(b64decode(b64file)) as file_to_upload:
            ...
            self.assertIn("http://graph.org/file/", telegraph_image_url)

    def test_mime_headers(self):
        self.assertEqual(
            "image/jpeg",
            _get_mimetype_from_response_headers({"Content-Type": "image/jpg"}),
        )
        self.assertEqual(
            "image/jpeg",
            _get_mimetype_from_response_headers({"Content-Type": "image/jpeg"}),
        )

    def test_get_delay_timeout(self):
        def _upload():
            upload_image(
                "http://httpbin.org/delay/3", return_json=True, get_timeout=(2, 2)
            )
...
class TelegraphPosterNoApiTest(unittest.TestCase):
    def test_post(self):
        ...
        self.assertTrue(
            "url" in result
            and "path" in result
            and "tph_uuid" in result
            and "page_id" in result
        )


class TelegraphPosterApiTest(unittest.TestCase):
    def setUp(self):
        # Access Token from graph.org/api page
    ...
    def test_api_token(self):
        ...
        self.assertEqual("teleposter_test", result["short_name"])
        self.assertEqual("tele_author_test", result["author_name"])
    ...
    def test_edit_with_path(self):
        ...
        result2 = t2.edit(
            title="test_page0201_edit",
            text="<p>test paragraph edited</p>",
            path=result["path"],
        )
    ...
    def test_edit_account_info(self):
        ...
        acc_info = t.edit_account_info(
            short_name="Sandbox", author_name="aaa", author_url="https://telegram.org/"
        )
        self.assertEqual(acc_info["short_name"], "Sandbox")
