...
class DocumentPreprocessorImageUploadTest(unittest.TestCase):
    ...
    def test_upload_all_images(self):
        html_images = self.document.format(
            headcontent="",
            bodycontent='<img src="http://graph.org/file/test.jpeg" /> '
            + self.html_figures.format(
                "http://httpbin.org/image/jpeg", "http://httpbin.org/image/png"
            ),
        )
        ...
        self.assertEqual(
            3,
            len(
                dp.parsed_document.body.xpath(
                    './/img[starts-with(@src, "http://graph.org/file/")]'
                )
            ),
        )
        self.assertTrue('<img src="http://graph.org/file/' in dp.get_processed_html())
    ...
    def test_make_links_absolute_document_base_url(self):
        ...
        self.assertTrue('<img src="http://httpbin.org/image/png">' in processed_html)
        self.assertTrue('<img src="http://httpbin.org/image/jpeg">' in processed_html)
    ...
    def test_make_links_absolute_document_without_base_url(self):
        ...
        dp._make_links_absolute()
        processed_html = dp.get_processed_html()
        self.assertFalse("<img src" in processed_html)
        self.assertFalse("<img src" in processed_html)

    def test_make_links_absolute_pass_base_url(self):
        html_images = self.document.format(
            headcontent="",
            bodycontent='<a href="test_link.html">test Link</a>'
            + self.html_figures.format("image/jpeg", "image/png"),
        )
        dp = DocumentPreprocessor(html_images)
